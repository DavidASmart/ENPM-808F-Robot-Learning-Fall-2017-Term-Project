...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% initialize Q-matrix
Q = zeros(numrc*2, numrc*2, 2^4, 5, 5);

% bootstrap Q-learning with premade policy
% QInit_ca; % avoid collisions
% QInit_ca_dg2; % avoid collisions & move toward goal
% QInit_ca_dg_ocd;  % avoid collisions & move toward goal ...
                    % ... try moving "inteligently" (forward) if oscillating 
                    % (direction to goal considered, but direction of ocd not considered)
                    % may be suboptimal...still results in timeouts if
                    % blockage is too large or "cup shaped"
                    
% learning rate (alpha)
% initialize state-action counter (used to calculate alpha)
saCounter = ones(numrc*2, numrc*2, 2^4, 5, 5);
...
% some stats
timeouts = zeros(1,numepisodes);
numcrashes = zeros(1,numepisodes);
goalsreached = zeros(1,numepisodes);
EvalErr = 0;
